...
class Lexer
  ...
  def read_token
    ...
    when '~' then read_char(char, :INVERT)
    when '&' then read_char(char, :BITAND)
    when '|' then read_char(char, :BITOR)
    when '^' then read_char(char, :BITXOR)
...
class Interpreter
  ...
  def bitwise
    ...
    while %i[BITAND BITOR BITXOR].include?(@current_token.type)
      token = @current_token
      eat(token.type)

      case token.type
      when :BITAND then result = (result.to_i & unary.to_i).to_f
      when :BITOR then result = (result.to_i | unary.to_i).to_f
      when :BITXOR then result = (result.to_i ^ unary.to_i).to_f
      end
    end
  ...
  def term
    ...
    while %i[MUL DIV MOD].include?(@current_token.type)
      ...
      case token.type
      when :MUL then result *= bitwise
      when :DIV then result /= bitwise
      when :MOD then result %= bitwise
      end
    end
  ...
  def eat(type)
    if @current_token.type == type
      ...
      raise <<-END_OF_MESSAGE
        unexpected token #{@current_token.type} at index #{@lexer.pos}:
        expected token of type #{type}
      END_OF_MESSAGE
    else
      raise <<-END_OF_MESSAGE
        unexpected token #{@current_token.type}
        (value = #{@current_token.value}) at index #{@lexer.pos}:
        expected token of type #{type}
      END_OF_MESSAGE
    end
  end
